...
#The goal of this analysis script is to compare learning, between mice started on the pulse rule vs the frequency rule, binned by day.
...
for mouse in range(len(mouse_list)):
    ...
    for date in range(len(date_list)):
        ...
        for block in range(len(block_list)):
            ...
            performance_block_day = sum(performance_block)/total_trials
            performance[mouse] = np.append(performance[mouse], performance_block_day) #add performance data for this block


    init_days = 8 #number of days to analyze
    ...
    init_performance = performance[mouse][:init_days]

    if init_rule[0] == 1: #Started on frequency rule

        all_freq_learning.append(init_performance)
        #conv_perform =  conv_perform = gaussian_filter1d(init_performance, sigma=5, mode='mirror')

        if len(mouse_list) > 1:

            ax.plot(init_performance, color = 'red', alpha=0.5, lw=0.75)

        else:

            ax.plot(init_performance, color='red', label='Initial Training on Frequency Rule')
    ...
    if init_rule[0] == 0: #Started on pulse rule
        ...
        if len(mouse_list) > 1:
            ...
            ax.plot(init_performance, color = 'blue', alpha=0.5, lw=0.75)

        else:

            ax.plot(init_performance, color='blue', label='Initial Training on Pulse Rule')

if len(mouse_list) > 1:
    
    freq_learning_array = np.empty((len(all_freq_learning), init_days))
    freq_learning_array.fill(np.nan)
    pulse_learning_array = np.empty((len(all_pulse_learning), init_days))
    pulse_learning_array.fill(np.nan)

    for mouse in range(len(all_freq_learning)):

        freq_learning_array[mouse,:len(all_freq_learning[mouse])] = all_freq_learning[mouse]

    for mouse in range(len(all_pulse_learning)):

        pulse_learning_array[mouse,:len(all_pulse_learning[mouse])] = all_pulse_learning[mouse]

    freq_avg = np.nanmean(freq_learning_array, axis=0)
    pulse_avg = np.nanmean(pulse_learning_array, axis=0)

    ax.plot(freq_avg, color='red', label=f'Initial Training on Frequency Rule (n = {len(all_freq_learning)})', lw=2)
    ax.plot(pulse_avg, color='blue', label=f'Initial Training on Pulse Rule (n = {len(all_pulse_learning)})', lw=2)


ax.set_xlabel('Training_days')
